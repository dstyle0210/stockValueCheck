/**
 * 종목정보 오브젝트 형식
 * @typedef {code:string,cap:string,rim:string,equity:string,stocks:string,profit:string,dividend:string,psr:string} MddData
 */
type MddData = {
    code:string, // 종목코드
    percent?:string, // 
...
module.exports = async (page:any,codeList:string[]) : Promise<MddData[]> => {
    let result : MddData[] = []; // 최종리턴
     ...
     * @return {Promise<MddData | null>}
     */
    async function getData(stockCode:string) : Promise<MddData | null> {
        ...
        if(Number(stock.min) < -0.31 || Number(stock.max) > 0.31){
            return {
                code:stockCode,
                percent:"편차오류",
                high:"편차오류"
